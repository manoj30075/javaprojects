public class Bishop3D extends Bishop {
        ...
        for (int i = 1; i <= board3D.MAX_ROWS; i++) {
            for (int j = 1; j <= board3D.MAX_COLUMNS; j++) {
                for (int k = 1; k <= board3D.MAX_WIDTH; k++) {
                    if (Math.abs(i-j) == (Math.abs(thisRow - thisColumn)) ||
                            Math.abs(k-j) == (Math.abs(thisWidth - thisColumn)) ||
                            Math.abs(i-k) == (Math.abs(thisRow - thisWidth)) ||
                            Math.abs(i-j-k) == (Math.abs(thisRow - thisColumn - thisWidth))
                    ) {
    ...
    public void placeBishop(int startC, int startR, int startW, Board3D board3D)	{
